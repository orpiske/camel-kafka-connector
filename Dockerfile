...
FROM registry.access.redhat.com/ubi8/ubi-minimal AS camel-kafka-connector
ARG KAFKA_VERSION
ENV KAFKA_VERSION ${KAFKA_VERSION:-2.4.0}
...
RUN echo "\$KAFKA_HOME/bin/connect-standalone.sh \$KAFKA_HOME/custom-config/connect-standalone.properties \$KAFKA_HOME/custom-config/CamelConnector.properties" >> /opt/run-connector.sh
